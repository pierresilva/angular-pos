...
export class EmailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    $('body').addClass('email-application');
    ...
    if ($('.sidebar-menu-list').length > 0) {
      const sidebar_menu_list = new PerfectScrollbar('.sidebar-menu-list');
    }

    // User list scroll
    if ($('.email-user-list').length > 0) {
      const users_list = new PerfectScrollbar('.email-user-list');
    }

    // Email detail section
    if ($('.email-scroll-area').length > 0) {
      const users_list = new PerfectScrollbar('.email-scroll-area');
    }

    // Modal dialog scroll
    if ($('.modal-dialog-scrollable .modal-body').length > 0) {
      const sidebar_menu_list = new PerfectScrollbar('.modal-dialog-scrollable .modal-body');
    }
    ...
    $('.email-app-list #email-search').on('keyup', function() {
      ...
      if (value != '') {
        ...
        const tbl_row = $('.email-user-list .users-list-wrapper li:visible').length; // here tbl_test is table name
    ...
    const emailEditor = new Quill('#email-container .editor', {
